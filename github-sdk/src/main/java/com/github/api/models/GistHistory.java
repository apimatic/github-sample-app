/*
 * GitHubAPILib
 *
 * This file was automatically generated by APIMATIC v3.0 ( https://www.apimatic.io ).
 */

package com.github.api.models;

import com.fasterxml.jackson.annotation.JsonGetter;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonSetter;
import com.fasterxml.jackson.databind.annotation.JsonDeserialize;
import com.fasterxml.jackson.databind.annotation.JsonSerialize;
import com.github.api.DateTimeHelper;
import io.apimatic.core.types.BaseModel;
import java.time.LocalDateTime;

/**
 * This is a model class for GistHistory type.
 */
public class GistHistory {
    private Object user;
    private String version;
    private LocalDateTime committedAt;
    private ChangeStatus changeStatus;
    private String url;

    /**
     * Default constructor.
     */
    public GistHistory() {
    }

    /**
     * Initialization constructor.
     * @param  user  Object value for user.
     * @param  version  String value for version.
     * @param  committedAt  LocalDateTime value for committedAt.
     * @param  changeStatus  ChangeStatus value for changeStatus.
     * @param  url  String value for url.
     */
    public GistHistory(
            Object user,
            String version,
            LocalDateTime committedAt,
            ChangeStatus changeStatus,
            String url) {
        this.user = user;
        this.version = version;
        this.committedAt = committedAt;
        this.changeStatus = changeStatus;
        this.url = url;
    }

    /**
     * Getter for User.
     * @return Returns the Object
     */
    @JsonGetter("user")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public Object getUser() {
        return user;
    }

    /**
     * Setter for User.
     * @param user Value for Object
     */
    @JsonSetter("user")
    public void setUser(Object user) {
        this.user = user;
    }

    /**
     * Getter for Version.
     * @return Returns the String
     */
    @JsonGetter("version")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public String getVersion() {
        return version;
    }

    /**
     * Setter for Version.
     * @param version Value for String
     */
    @JsonSetter("version")
    public void setVersion(String version) {
        this.version = version;
    }

    /**
     * Getter for CommittedAt.
     * @return Returns the LocalDateTime
     */
    @JsonGetter("committed_at")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    @JsonSerialize(using = DateTimeHelper.Rfc8601DateTimeSerializer.class)
    public LocalDateTime getCommittedAt() {
        return committedAt;
    }

    /**
     * Setter for CommittedAt.
     * @param committedAt Value for LocalDateTime
     */
    @JsonSetter("committed_at")
    @JsonDeserialize(using = DateTimeHelper.Rfc8601DateTimeDeserializer.class)
    public void setCommittedAt(LocalDateTime committedAt) {
        this.committedAt = committedAt;
    }

    /**
     * Getter for ChangeStatus.
     * @return Returns the ChangeStatus
     */
    @JsonGetter("change_status")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public ChangeStatus getChangeStatus() {
        return changeStatus;
    }

    /**
     * Setter for ChangeStatus.
     * @param changeStatus Value for ChangeStatus
     */
    @JsonSetter("change_status")
    public void setChangeStatus(ChangeStatus changeStatus) {
        this.changeStatus = changeStatus;
    }

    /**
     * Getter for Url.
     * @return Returns the String
     */
    @JsonGetter("url")
    @JsonInclude(JsonInclude.Include.NON_NULL)
    public String getUrl() {
        return url;
    }

    /**
     * Setter for Url.
     * @param url Value for String
     */
    @JsonSetter("url")
    public void setUrl(String url) {
        this.url = url;
    }

    /**
     * Converts this GistHistory into string format.
     * @return String representation of this class
     */
    @Override
    public String toString() {
        return "GistHistory [" + "user=" + user + ", version=" + version + ", committedAt="
                + committedAt + ", changeStatus=" + changeStatus + ", url=" + url + "]";
    }

    /**
     * Builds a new {@link GistHistory.Builder} object.
     * Creates the instance with the state of the current model.
     * @return a new {@link GistHistory.Builder} object
     */
    public Builder toBuilder() {
        Builder builder = new Builder()
                .user(getUser())
                .version(getVersion())
                .committedAt(getCommittedAt())
                .changeStatus(getChangeStatus())
                .url(getUrl());
        return builder;
    }

    /**
     * Class to build instances of {@link GistHistory}.
     */
    public static class Builder {
        private Object user;
        private String version;
        private LocalDateTime committedAt;
        private ChangeStatus changeStatus;
        private String url;



        /**
         * Setter for user.
         * @param  user  Object value for user.
         * @return Builder
         */
        public Builder user(Object user) {
            this.user = user;
            return this;
        }

        /**
         * Setter for version.
         * @param  version  String value for version.
         * @return Builder
         */
        public Builder version(String version) {
            this.version = version;
            return this;
        }

        /**
         * Setter for committedAt.
         * @param  committedAt  LocalDateTime value for committedAt.
         * @return Builder
         */
        public Builder committedAt(LocalDateTime committedAt) {
            this.committedAt = committedAt;
            return this;
        }

        /**
         * Setter for changeStatus.
         * @param  changeStatus  ChangeStatus value for changeStatus.
         * @return Builder
         */
        public Builder changeStatus(ChangeStatus changeStatus) {
            this.changeStatus = changeStatus;
            return this;
        }

        /**
         * Setter for url.
         * @param  url  String value for url.
         * @return Builder
         */
        public Builder url(String url) {
            this.url = url;
            return this;
        }

        /**
         * Builds a new {@link GistHistory} object using the set fields.
         * @return {@link GistHistory}
         */
        public GistHistory build() {
            return new GistHistory(user, version, committedAt, changeStatus, url);
        }
    }
}
